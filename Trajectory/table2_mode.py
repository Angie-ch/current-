...
- 先对 N 条预测轨迹做 ensemble average。
- 再对 ensemble mean trajectory 计算逐时效 FDE。

说明：
- 默认按 6h 间隔报告误差，以对齐论文表格。
- 可走 end_to_end（实时扩散采样）或 from_saved（读取 ar_pred_*.pt）。
"""
...
def run_end_to_end(args) -> List[Dict[str, np.ndarray]]:
    ...
    for storm_id in all_typhoon_ids:
        ...
        if not (lat_ok and lon_ok):
            ...
            continue
        # Length check (already have count)
    ...
    # Build dataset with all valid typhoons (use ALL samples, not just test split)
    ...
    # Build sample index mapping from dataset
    ...
    for count, storm_id in enumerate(selected_typhoons, 1):
        ...
        # 批量采样: 将 num_samples 次串行 DDIM 改为分批并行
        ...
        while num_done < args.num_samples:
            ...
            # 复制条件 n_this_batch 份, 每份走独立 DDIM 噪声路径
            cond_batched = cond.repeat(n_this_batch, 1, 1, 1)  # (n, C*T_hist, H, W)
            ...
            # preds: list of (n, C, H, W), len = t_future

            for b in range(n_this_batch):
                # 提取第 b 个样本的预测
                preds_b = [p[b:b+1] for p in preds]  # list of (1, C, H, W)
